...
class SharePointMatcher:
    ...
    def stage_area_prospeccao(self, data_inicio, data_fim):
        ...
        try:
            ...
            # Ensure data_inicio is a datetime object
            data_inicio = pd.to_datetime(data_inicio)
            
            # Remove rows with invalid dates
            df = df.dropna(subset=['data_prospeccao'])
            
            # Filter rows by the specified date range
            df = df[(df['data_prospeccao'] >= data_inicio)]
    ...
    def prospeccao_id_gerem_causal_provavel(self):
        ...
        try:
            ...
            # Ensure date columns are datetime objects
            df_validado['data_interacao'] = pd.to_datetime(df_validado['data_interacao'])
            df_validado['data_prospeccao'] = pd.to_datetime(df_validado['data_prospeccao'])
    ...
    def output_prospeccao(self):
        ...
        try:
            ...
            # Filter only records with validacao_verossimilhanca = "Sim"
            ...
            if 'id_gerem_causal_provavel' in df_prospeccao_apurado_e_analisado.columns:
                ...
                # Filter records where id_gerem == id_gerem_causal_provavel if both columns exist
